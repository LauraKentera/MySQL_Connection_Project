...
# Question 3
SELECT
    CONCAT(p.LName, ', ', p.FName) AS "Passenger"
FROM trip_people tp
...
FROM trip_people tp
         JOIN passenger p ON tp.PassengerID = p.PassengerID
         JOIN zips z ON p.Zip = z.Zip
WHERE tp.TripNum = '1027' AND tp.Date = '2023-10-11';
...
# Question 8
SELECT
    DISTINCT e.EquipmentDescription AS "Equipment Description"
FROM trip_people tp
         JOIN passenger p ON tp.PassengerID = p.PassengerID
         JOIN trip t ON tp.TripNum = t.TripNum AND tp.Date = t.Date
         JOIN equipment e ON t.EquipID = e.EquipID
WHERE p.FName = 'Mark' AND p.LName = 'Lucas';

# Question 9
SELECT
    COUNT(DISTINCT tp.PassengerID) AS "Number of Bus Passengers"
FROM trip_people tp
         JOIN trip t ON tp.TripNum = t.TripNum AND tp.Date = t.Date
         JOIN trip_directory td ON t.TripNum = td.TripNum
         JOIN tripcodes tc ON td.TripType = tc.TripType
...
# Question 12
SELECT DISTINCT EquipID FROM trip;
...
# All trips in August
SELECT * FROM trip WHERE MONTH(Date) = 8;
...
# More than one trip
SELECT
    tc.TypeName AS "Transportation Type",
    COUNT(t.TripNum) AS "Number of Trips"
FROM trip t
         JOIN trip_directory td ON t.TripNum = td.TripNum
         JOIN tripcodes tc ON td.TripType = tc.TripType
